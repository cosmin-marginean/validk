...
class Validation<T>(
    ...
    infix fun <R> Constraint<R>.message(errorMessage: String) {
    ...
    fun validate(value: T?): ValidationResult {
        if (value == null) {
            ...
            return ValidationResult(propertyPath!!, nullMessage)
        }
        ...
        if (dynamicValidations.isNotEmpty()) {
            ...
            errors.addAll(dynamicValidation.validate(value).errors)
        }
        ...
        childValidations.forEach { (property, validation) ->
            ...
            if (validation.checksCollectionElements) {
                (propertyValue as Collection<*>).forEachIndexed { index, element ->
                    errors.addAll(validation.validate(element!!).errors.map { it.indexed(property.name, index) })
                }
            } else {
                errors.addAll(validation.validate(propertyValue).errors)
            }
        }
...
inline fun <reified T> T.validate(init: Validation<T>.() -> Unit): ValidationResult {
